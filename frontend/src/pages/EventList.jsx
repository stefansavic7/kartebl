...
/*
  ...
  */
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const mappedEvents = response.data.map(event => ({
          ...
          date: event.datum,
          image: event.slika 
            ? `data:${event.tip_slike};base64,${event.slika}` // Konverzija slike
            : "default-image.jpg", // Ako nema slike, koristi default
          approved: event.odobren,
        }));

        setEvents(mappedEvents); // Postavljanje podataka u stanje
      } catch (error) {
    ...
    fetchEvents();
  }, [token]); // Pokreni efekat kada se promeni token
